# Break Down Doc Into Implementation Tasks

You are breaking down a PRD or feature Markdown doc into specific, implementable tasks. The primary goal is to create individual files for each identified task and add them to the relevant folder for proper tracking and implementation workflow.

**Task:** Break down Markdown doc $ARGUMENTS into implementation task files.

## Step 1: Fetch and Analyze Source Issue

1. Fetch the source markdown doc details using the filename/path from $ARGUMENTS. If only the filename or ID is provided, look in `docs/{id}-{description}`. The filename with either be `[Feature] {...}.md` or `[PRD] {...}.md`.
2. Read the markdown file and all requirements
3. Identify the issue type (PRD vs Feature) from the file name
...
## Step 5: Create Task Issues with Parent Relationships

**Use this title pattern:** `[Task {Number}] {Area}: {Specific Implementation}.md`
...
**Use this template for each markdown body:**

```markdown
# Task {Number}: {Specific Implementation}

**Documentation:** {link to prd or feature}
...
## Step 7: Update Parent Markdown Doc

1. Add a task breakdown summary to the markdown doc
...
## Step 10: Deliver Issues and Summary

**Primary Output: Created Issues**
Ensure all identified task issues have been created and properly linked to the parent doc. Each issue should be immediately actionable with clear acceptance criteria and implementation details.

**Create a detailed breakdown summary:**

- **Parent Doc**: Markdown path and full title
- **Issues Created**: List of all created task issue numbers with hierarchical numbering
...
- ✅ All identified tasks are captured as individual Markdown files
- ✅ Parent doc has been updated with numbered task checklist
...
This systematic approach ensures comprehensive task breakdown while maintaining clear traceability through the file system, with the primary deliverable being a complete set of actionable issues.
